...
import { execLnd, loadBakeBoyData, saveBakeBoyData, createBakeBoyUri } from "./util";
...
export function list() {
  ...
  const fields = [{
    ...
    label: 'Date created',
    len: 15,
    transform: (bb: BakeBoy) => new Date(bb.dateCreated).toLocaleDateString()
  }, {
    label: 'Date expiring',
    len: 15,
    transform: (bb: BakeBoy) => bb.dateTimeout ? new Date(bb.dateTimeout).toLocaleDateString() : 'N/A',
  }];
  let titles = '';
  let totalLen = 0;
...
export async function bake({ label, timeout, canSend, canReceive}: BakeArgs) {
  ...
  try {
    ...
    console.error(err);
    console.error(`Failed to save bake boy, but macaroon was created: ${macaroon}`);
    process.exit(1);
  }
