...
export function loadBakeBoyConf(args: Partial<BakeBoyConf> = {}) {
  ...
  if (!pathExists) {
    console.info(`${CONF_PATH} doesn't exist, setting you up with a fresh one...`);
...
export function loadBakeBoyData() {
  ...
  if (!fs.existsSync(DATA_PATH)) {
    console.info(`${DATA_PATH} doesn't exist, setting you up with a fresh one...`);
...
export async function createBakeBoyUri(macaroon: string) {
  const conf = getConf();
  const cert = fs.readFileSync(path.resolve(conf.tlscert)).toString('base64');
  return `bakeboy://${conf.grpcurl}?cert=${cert}&macaroon=${macaroon}`;
}
